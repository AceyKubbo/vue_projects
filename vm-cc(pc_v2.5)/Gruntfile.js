module.exports = function(grunt) {
    ...
    var styleBundle = [];
    if (build.mode == 'dev') {
        if(grunt.file.exists("dist/css/lib")&&grunt.file.exists("dist/css/" + buildDir)){// 针对第一次创建项目没有生成对应的目录的判断,创建相应的目录
            ...
            if(grunt.file.exists("dist/assets")){
                grunt.file.recurse("dist/assets", function (abspath, rootdir, subdir, filename) {
                    if (filename.indexOf('\.css') >0 && filename.indexOf('\.min\.css') == -1 ) {
                        // console.info(abspath,subdir);
                        // styleBundle.push('dist/assets/' + subdir + "/" + filename)
                    }
                    styleBundle.push('dist/css/lib/' + filename)
                });
            }else {
    ...
    ['lib'].forEach(function(jsLibDir){
        grunt.file.recurse("js/" + jsLibDir,function(abspath, rootdir, subdir, filename){
    ...
    grunt.initConfig({
      ...
      concat: {
          javascript: {
              src: ['js/' + buildDir + '/main.js','js/' + buildDir + '/layout/*.js','js/' + buildDir + '/mods/*.js'],
              dest: 'dist/js/' + buildDir + '/'+jsName+'.js'
          },
          css : {
              // src: ['src/css/lib/bootstrap.css','src/css/lib/layer.css','src/css/' + buildDir + '/' + styleName + '.css'],
      ...
      cssmin:{
          css: {
              files: [
                  // {expand: true,cwd: 'dist/css',src: ['**/*.min.css'],dest: 'dist/css'}
    ...
    grunt.event.on('watch', function(action, filepath, target) {
        ...
        if(filepath.lastIndexOf('\.html') != -1 ){
            ...
            if(filepath.lastIndexOf('\\template\\') == -1) {
                var jsFilePath = filepath.replace('src\\html', 'js').replace('\.html', '\.js');
                if (!grunt.file.exists(jsFilePath)) {
                    var fileName = filepath.substring(filepath.lastIndexOf("\\") + 1);
                    grunt.log.ok("生成js文件路径--->"+jsFilePath+"--->fileName:"+fileName)
                    grunt.file.write(jsFilePath,
                        grunt.template.process(grunt.file.read('js/template/blankTemplate.js'), {
                            data: {
                                pageName: fileName
                            }
                        })
            ...
            if(build.mode=="dev"){
                ...
                //js 目录
                var jsDir = "js/" + buildDir;
                if (!grunt.file.exists(jsDir)) {
                    grunt.file.mkdir(jsDir);
                    grunt.file.write(jsDir+"/main.js",
                        grunt.template.process(grunt.file.read('src/js/template/mainTemplate.js'), {
                            data: {}
                        })
                    );
                }
