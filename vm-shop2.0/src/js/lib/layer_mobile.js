...
//点触事件
    ...
    Layer.prototype.view = function(){
        ...
        var button = (function(){
            ...
            //大于2个按钮
            else if(config.btnsFun){
                btndom='';
                for(var i=0;i<config.btn.length;i++){
                    btndom = btndom +'<span yes>'+ config.btn[i] +'</span>';
                }
            }
    ...
    Layer.prototype.action = function(config, elem){
        ...
        if(config.btn&&config.btnsFun){
            if(config.btn.length==config.btnsFun.length){
                var btns = elem[claname]('layui-m-layerbtn')[0].children
                for(var p=0;p<btns.length;p++){
                    ready.touch(btns[p], config.btnsFun[p]);
                }
            }else{
                console.error('按钮数和按钮方法数必须保持一致!')
                return;
            }
        }
        //点遮罩关闭
